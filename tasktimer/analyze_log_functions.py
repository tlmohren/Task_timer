...
def plot_week_tasks(ax_pl, df_week, label_dict, bool_legend=False):
    ...
    ax_pl.set_xlim(matplotlib.dates.date2num(mon_day) - 0.2,
                   matplotlib.dates.date2num(sun_day) + 0.2)
    ...
    minmax_sec = np.array(
        [start_time.min(), (start_time + df_week['Duration (s)']).max()])
    ...
    if bool_legend:
        ...
        ax_pl.legend(by_label.values(), by_label.keys(),
                     loc='center left', bbox_to_anchor=(1, 0.5))

    bbox_props = dict(boxstyle="round,pad=0.1",
                      fc="w", ec="w", lw=2, alpha=0.5)
...
def plot_time_spent_weekly(ax_pl, df_pl, label_dict, bool_legend=False):
    ...
    ax_pl.xaxis.set_ticklabels(hours_to_hhmm(
        np.int64(ticks / 3600), bool_h=True))
...
def pick_mostrecent_log(log_dir):
    file_dir = str(log_dir) + os.path.sep + '*.csv'
    log_files = sorted(glob.glob(file_dir))

    return log_files[-1]

def weekly_data_processing(file):

    df = pd.read_csv(file, index_col=None, header=0)

    # remove any task shorter than 1 minute,
